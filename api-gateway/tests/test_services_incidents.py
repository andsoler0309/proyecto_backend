...
def test_create_incident(client):
    ...
    with patch('auth.decode_jwt') as mock_decode_jwt, \
        ...
        mock_requests_get.return_value.json.return_value = {'id': 'agent123', 'role': 'AGENT'}
...
def test_delete_incident(client):
    ...
    with patch('auth.jwt.decode') as mock_jwt_decode, \
         patch('requests.get') as mock_requests_get, \
         patch('requests.delete') as mock_requests_delete, \
         patch("models.Session.query") as mock_session_query:
        ...
        mock_requests_get.return_value.status_code = 200
        mock_requests_get.return_value.json.return_value = {'id': 'admin123', 'role': 'ADMIN'}

        # Mock the incident exists
        mock_incident_response = MagicMock()
        mock_incident_response.status_code = 200
        mock_incident_response.json.return_value = {'incident_id': 'incident123', 'agent_id': 'agent123'}
        mock_requests_get.side_effect = [mock_requests_get.return_value, mock_incident_response]
...
def test_get_incidents_by_user(client):
    with patch('requests.get') as mock_requests_get:
        ...
        mock_requests_get.return_value.status_code = 200
        mock_requests_get.return_value.json.return_value = [
            {'incident_id': 'incident123', 'agent_id': 'agent123', 'description': 'Test incident', 'date': '2023-01-01'},
            {'incident_id': 'incident456', 'agent_id': 'agent123', 'description': 'Another incident', 'date': '2023-01-02'}
        ]
...
def test_update_incident(client):
    ...
    with patch('auth.decode_jwt') as mock_decode_jwt, \
         ...
         patch('requests.get') as mock_requests_get, \
         patch("models.Session.query") as mock_session_query, \
         patch('requests.put') as mock_requests_put:
        ...
        mock_get_incident_response.json.return_value = {'incident_id': 'incident123', 'agent_id': 'agent123', 'description': 'Old description'}
        ...
        mock_requests_put.return_value.json.return_value = {'incident_id': 'incident123', 'agent_id': 'agent123', 'description': 'Updated description'}
...
def test_get_incident_possible_solution(client):
    with patch('requests.get') as mock_requests_get:
        ...
        mock_requests_get.return_value.status_code = 200
        mock_requests_get.return_value.json.return_value = {'incident_id': 'incident123', 'possible_solution': 'Restart the device'}
